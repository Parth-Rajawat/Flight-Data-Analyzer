...
input_file_path = "/Users/parthmac/Desktop/Projects/Flight Data Engineering/data/raw data/flights_raw_data.json"
output_file_path = "/Users/parthmac/Desktop/Projects/Flight Data Engineering/data/raw data/flights_raw_data.json"
number_of_new_records = 10000

# Load existing data (handle empty or missing file)
...
# If file is empty (data is []), generate full 10-day window
if not existing_data["data"]:
    print("📂 Empty file detected → Initializing with 10 days of data...")

    start_date = today - timedelta(days=11)
    end_date = yesterday

    for i in range(10):  # Generate 10 days (yesterday back to 10 days ago)
        flight_date = (end_date - timedelta(days=(9 - i))).strftime("%Y-%m-%d")
        new_records = [generate_flight_record(flight_date) for _ in range(number_of_new_records)]
        existing_data["data"].extend(new_records)
        print(f"🆕 Generated {number_of_new_records} records for {flight_date}")

    existing_data["pagination"]["count"] = len(existing_data["data"])
    existing_data["pagination"]["total"] = len(existing_data["data"])
    
    # Save updated JSON
    with open(output_file_path, "w") as f:
        json.dump(existing_data, f, indent=4)

    print(f"✅ Data initialized from {start_date} to {end_date}")

else:
    # Normal rolling behavior
    existing_dates = {rec["flight_date"] for rec in existing_data["data"]}

    if yesterday.strftime("%Y-%m-%d") in existing_dates:
        print(f"✅ Yesterday’s records ({yesterday}) already present. No changes made.")
    else:
        start_date = today - timedelta(days=10)
        end_date = yesterday

        # Step 1: Remove data older than start_date
        existing_data["data"] = [
            record for record in existing_data["data"]
            if start_date <= datetime.strptime(record["flight_date"], "%Y-%m-%d").date() <= end_date
        ]

        # Step 2: Generate new records for yesterday
        new_records = [generate_flight_record(yesterday.strftime("%Y-%m-%d")) for _ in range(number_of_new_records)]
        existing_data["data"].extend(new_records)
        print(f"🆕 Generated {number_of_new_records} records for {yesterday}")
        print(f"📊 Data now contains flights from {start_date} to {end_date}")
